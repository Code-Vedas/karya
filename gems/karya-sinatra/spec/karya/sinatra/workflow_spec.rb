...
RSpec.describe Karya::Sinatra::Workflow do
  ...
  it 'memoizes the workflow facade with the Sinatra configuration provider' do
    allow(Karya).to receive(:queue_store).and_return(instance_double(Karya::QueueStore::Base))
    allow(Karya::Sinatra).to receive(:configuration).and_return(:configuration)
