...
require_relative 'sinatra/job'
require_relative 'sinatra/rake_tasks'
require_relative 'sinatra/workflow'
  ...
  module Sinatra
    ...
    def runtime_probe_payload(backend_class: nil, backend_options: nil, scope: nil)
      ...
      Karya::FrameworkRuntime.shared_runtime(backend_class:, backend_options:).runtime_probe_payload(
        job_prefix: 'sinatra-modular-probe',
        worker_id: 'sinatra-modular-probe-worker',
...
# :nocov:
Karya::Sinatra::RakeTasks.load! if defined?(Rake.application)
# :nocov:
