...
module Karya
  module Sinatra
    # Registers framework-native Karya rake tasks for Sinatra apps.
    module RakeTasks
      module_function
      ...
      def register_work_task
        ::Rake::Task.define_task('karya:work', [:queues]) do |_task, args|
          Karya::Sinatra.load_config_file!
          Karya::Sinatra.support.run_worker(
      ...
      def register_runtime_tasks
        register_runtime_task('inspect') { |queues| print_runtime_payload(Karya::Sinatra.runtime_inspect(queues:, name: worker_name)) }
        register_runtime_task('drain') { |queues| Karya::Sinatra.runtime_drain(queues:, name: worker_name) }
        register_runtime_task('force_stop') { |queues| Karya::Sinatra.runtime_force_stop(queues:, name: worker_name) }
      end
      ...
      def register_install_all_task
        ::Rake::Task.define_task('karya:install:all', [:backend]) do |_task, args|
          ...
          Karya::Internal::FrameworkInstaller.install!(
            framework_class_name: 'Karya::Sinatra',
            root_path: Dir.pwd,
            backend:,
            migration_kind:,
