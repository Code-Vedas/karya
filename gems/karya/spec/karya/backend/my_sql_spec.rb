...
RSpec.describe Karya::Backend::MySQL do
  it 'exposes the backend identifier' do
    expect(described_class.new(url: 'mysql2://root@127.0.0.1/karya').identifier).to eq('mysql')
  end

  it 'builds the owned MySQL queue store with forwarded options' do
    queue_store = instance_double(Karya::QueueStore::MySQL)
    allow(Karya::QueueStore::MySQL).to receive(:new).and_return(queue_store)

    result = described_class.new(
      url: 'mysql2://root@127.0.0.1/karya',
      namespace: 'payments',
      max_batch_size: 50
    ).build_queue_store

    expect(result).to be(queue_store)
    expect(Karya::QueueStore::MySQL).to have_received(:new).with(
      url: 'mysql2://root@127.0.0.1/karya',
      namespace: 'payments',
      max_batch_size: 50
    )
  ...
  it 'normalizes invalid keyword errors into backend configuration errors' do
    allow(Karya::QueueStore::MySQL).to receive(:new).and_raise(ArgumentError, 'unknown keywords: unsupported_option')
  ...
  it 'normalizes queue-store validation errors into backend configuration errors' do
    ...
    allow(Karya::QueueStore::MySQL).to receive(:new).and_raise(error)
