...
RSpec.describe Karya::WorkerSupervisor::RuntimeStateStore do
  ...
  it 'writes state and lock files with owner-only permissions' do
    store = described_class.new(configuration:, path: state_file, supervisor_pid: 12_345)

    store.write_running

    expect(File.stat(state_file).mode & 0o777).to eq(0o600)
    expect(File.stat("#{state_file}.lock").mode & 0o777).to eq(0o600)
    expect(File.stat(File.dirname(state_file)).mode & 0o777).to eq(0o700)
  end
  ...
  it 'raises a descriptive error when acquiring the state lock times out' do
    ...
    allow(File).to receive(:open).with("#{state_file}.lock", File::RDWR | File::CREAT, 0o600).and_yield(lock_file)
