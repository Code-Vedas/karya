...
RSpec.describe 'Karya::Worker::HandlerExecution' do
  ...
  let(:framework_job_execution_class) { Karya::Worker.const_get(:FrameworkJobExecution, false) }
  let(:unsupported_execution_class) { Karya::Worker.const_get(:UnsupportedExecution, false) }
  let(:callable_handler) { -> {} }
  let(:framework_job_class) do
  ...
  it 'builds framework job execution for framework job classes' do
    execution = handler_execution_class.build(handler: framework_job_class, handler_name: 'billing_sync')

    expect(execution).to be_a(framework_job_execution_class)
  end
