...
RSpec.describe Karya::Primitives::OptionalCallable do
  subject(:optional_callable) { described_class.new(name, value, error_class:) }
  ...
  describe '#normalize' do
    context 'with nil value' do
      it 'returns nil' do
        result = described_class.new(name, nil, error_class:).normalize
    ...
    context 'with custom error class' do
      ...
      it 'raises the custom error class' do
        expect do
          described_class.new(name, 'not_callable', error_class:).normalize
        end.to raise_error(StandardError, 'callback must respond to #call')
      end
    end
    ...
    context 'with custom name' do
      ...
      it 'includes the custom name in error message' do
        expect do
          described_class.new(name, 'not_callable', error_class:).normalize
        end.to raise_error(ArgumentError, 'on_error must respond to #call')
      end
    end
  end
end
