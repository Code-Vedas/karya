...
RSpec.describe Karya::WorkerSupervisor do
  ...
  describe 'configuration validation' do
    it 'disconnects fork-aware queue stores before spawning child processes' do
      fork_safe_queue_store = Object.new
      disconnect_calls = 0
      fork_safe_queue_store.define_singleton_method(:disconnect_for_fork) do
        disconnect_calls += 1
      end
      local_supervisor = described_class.new(
        queue_store: fork_safe_queue_store,
        worker_id: 'worker-supervisor',
        queues: ['billing'],
        handlers: { 'billing_sync' => -> {} },
        lease_duration: 30,
      ...
      local_supervisor.send(
        :spawn_missing_children,
        {},
        1,
        described_class.const_get(:ShutdownController, false).new
      )

      expect(disconnect_calls).to eq(1)
    end
