...
RSpec.describe Karya::CLI do
  ...
  it 'raises a Thor error directly from runtime command helpers for invalid state files' do
    ...
    expect { command.drain }.to raise_error(Thor::Error, /does not exist/)
  end
  ...
  it 'raises a Thor error when the control server rejects the command' do
    ...
    expect do
      command.drain
    end.to raise_error(Thor::Error, 'runtime control failed: cannot drain now')
  ensure
    server_thread&.join
    FileUtils.rm_f(socket_file)
  end
  ...
  it 'raises a Thor error when the control server returns an unknown response shape' do
    ...
    expect do
      command.drain
    end.to raise_error(Thor::Error, 'runtime control failed: unknown runtime control error')
  ensure
    server_thread&.join
    FileUtils.rm_f(socket_file)
  end
  ...
  it 'times out when the control server accepts the request but never responds' do
    ...
    command = runtime_command_class.allocate
    allow(command).to receive(:options).and_return({ state_file: state_file })
    stub_const('Karya::Internal::FrameworkRuntimeControl::RESPONSE_TIMEOUT_SECONDS', 0)
    ...
    expect do
      command.drain
    end.to raise_error(Thor::Error, /runtime control failed: timed out waiting for supervisor response/)
  ensure
    server_thread&.join
    FileUtils.rm_f(socket_file)
  end
  ...
  it 'raises a Thor error when the control socket path points to a regular file' do
    ...
    expect do
      command.drain
    end.to raise_error(Thor::Error, /runtime control failed: .*not a Unix socket/i)
  end
end
