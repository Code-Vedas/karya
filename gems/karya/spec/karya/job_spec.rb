...
RSpec.describe Karya::Job do
  ...
  let(:enqueued_at) { Time.utc(2026, 3, 26, 12, 1, 0) }
  ...
  describe '#initialize' do
    it 'builds an immutable canonical job with normalized fields', :aggregate_failures do
      ...
      expect(job.enqueued_at).to eq(enqueued_at)
    ...
    it 'defaults enqueued_at to created_at when none is provided' do
      job = described_class.new(
        id: 'job123',
        queue: 'billing',
        handler: 'billing_sync',
      ...
      expect(job.enqueued_at).to eq(created_at)
    end
  ...
  describe '#transition_to' do
    ...
    it 'refreshes enqueued_at when transitioning back into queued state' do
      job = described_class.new(
        id: 'job_queued_refresh',
        queue: 'billing',
        handler: 'billing_sync',
        state: :retry_pending,
        created_at:,
        enqueued_at: created_at + 5,
        updated_at: created_at + 10,
        next_retry_at:
      )

      transitioned_job = job.transition_to(:queued, updated_at:)

      expect(transitioned_job.enqueued_at).to eq(updated_at)
      expect(transitioned_job.next_retry_at).to eq(next_retry_at)
    end

    it 'uses an explicit enqueued_at override when transitioning into queued state' do
      explicit_enqueued_at = updated_at + 30
      job = described_class.new(
        id: 'job_queued_override',
        queue: 'billing',
        handler: 'billing_sync',
        state: :retry_pending,
        created_at:,
      ...
      transitioned_job = job.transition_to(:queued, updated_at:, enqueued_at: explicit_enqueued_at)

      expect(transitioned_job.enqueued_at).to eq(explicit_enqueued_at)
    end

    it 'preserves an explicit enqueued_at override for non-queued transitions' do
      explicit_enqueued_at = updated_at + 45
      job = described_class.new(
        id: 'job_running_override',
        queue: 'billing',
        handler: 'billing_sync',
        state: :reserved,
        created_at:,
      ...
      transitioned_job = job.transition_to(:running, updated_at:, enqueued_at: explicit_enqueued_at)
  ...
  describe 'marshal hooks' do
    it 'round-trips job state while preserving custom lifecycle extensions' do
      ...
      expect(restored_job.enqueued_at).to eq(created_at)
    ...
    it 'rejects marshal_dump for non-registry lifecycle implementations' do
      ...
      expect do
        ...
        'queue-store persistence requires JobLifecycle::Registry lifecycles'
      )
    end
  end
end
