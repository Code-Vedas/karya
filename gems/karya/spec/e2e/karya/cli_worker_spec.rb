...
RSpec.describe Karya::CLI, :e2e, :integration do
  ...
  def parse_cli_json(output)
    JSON.parse(output.slice(output.index('{')..))
  end
  ...
  it 'inspects, drains, and force-stops a running worker through the runtime CLI' do
    Dir.mktmpdir('karya-cli-runtime-control') do |directory|
      state_file = File.join(directory, 'runtime.json')
      marker_file = File.join(directory, 'started.txt')
      draining_marker_file = File.join(directory, 'draining.txt')
      boot_file = build_force_stop_boot_file(directory:, marker_file:, draining_marker_file:)
      ...
      with_force_stop_worker(boot_file:, state_file:) do |_stdin, stdout_and_stderr, wait_thr|
        ...
        inspect_payload = parse_cli_json(inspect_stdout)
        ...
        drain_stdout, drain_stderr, drain_status = run_cli('runtime', 'drain', '--state-file', state_file)

        expect(drain_status.exitstatus).to eq(0), -> { "stdout:\n#{drain_stdout}\n\nstderr:\n#{drain_stderr}" }
        expect(wait_for_runtime_phase(state_file, 'draining')).to include('phase' => 'draining')

        force_stop_stdout, force_stop_stderr, force_stop_status = run_cli('runtime', 'force_stop', '--state-file', state_file)
