...
RSpec.describe Karya::CLI, :e2e, :integration do
  ...
  it 'boots a worker through the Redis backend path when a Redis server is configured' do
    redis_url = ENV.fetch('KARYA_REDIS_URL', ENV.fetch('REDIS_URL', nil))
    skip 'set KARYA_REDIS_URL or REDIS_URL for Redis e2e coverage' unless redis_url
