  ...
  class Job
    ...
    def transition_to(
      ...
      **overrides
      ...
      enqueued_at_override = overrides[:enqueued_at]
      explicit_enqueued_at = overrides.key?(:enqueued_at)
      transition_overrides = TransitionOverrides.new(self, overrides.except(:enqueued_at)).to_h
      normalized_next_state = lifecycle.validate_transition!(from: state, to: next_state)
      normalized_enqueued_at =
        if normalized_next_state == :queued
          explicit_enqueued_at ? enqueued_at_override : updated_at
        else
          explicit_enqueued_at ? enqueued_at_override : enqueued_at
        end
    ...
    def enqueued_at = lifecycle_state.enqueued_at
