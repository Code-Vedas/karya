  ...
  class Worker
    ...
    class HandlerExecution
      def self.build(handler:, handler_name:)
        return FrameworkJobExecution.new(handler) if framework_job_class?(handler)
      ...
      def self.framework_job_class?(handler)
        handler.is_a?(Class) && defined?(Karya::FrameworkJob::Base) && handler < Karya::FrameworkJob::Base
      end
