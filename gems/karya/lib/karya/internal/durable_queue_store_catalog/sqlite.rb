...
module Karya
  module Internal
    module DurableQueueStoreCatalog
      # SQLite durable schema definitions for the normalized queue-store model.
      module SQLite
        module_function
        ...
        def schema_sql
          create_statements.map { |statement| "#{statement.rstrip};" }.join("\n\n")
        end
        ...
        def metadata_sql
          <<~SQL
            CREATE TABLE IF NOT EXISTS #{Shared.table_name(:metadata)} (
              ...
              reservation_token_sequence integer NOT NULL DEFAULT 0,
