...
module Karya
  module Internal
    # Shared deep-normalization and freeze support for argument-style value graphs.
    class ImmutableArgumentGraph
      IMMUTABLE_SCALAR_CLASSES = [NilClass, Numeric, Symbol, TrueClass, FalseClass].freeze
      DUPLICABLE_SCALAR_CLASSES = [String, Time].freeze

      def initialize(arguments, error_class:)
        @arguments = arguments
        @error_class = error_class
      end

      def normalize
        raise error_class, 'arguments must be a Hash' unless arguments.is_a?(Hash)
        ...
        freeze_hash(arguments, tracker: TraversalTracker.new(error_class:))
      end
      ...
      attr_reader :arguments, :error_class
      ...
      def freeze_hash(value, tracker:)
        tracker.around(value) do
          value.each_with_object({}) do |(key, item), normalized|
            ...
            raise error_class, 'argument keys must be present' if normalized_key.empty?

            normalized_key = normalized_key.freeze
            raise error_class, "duplicate argument key after normalization: #{normalized_key.inspect}" if normalized.key?(normalized_key)
      ...
      class TraversalTracker
        ...
        def track(value)
          ...
          raise error_class, 'arguments must not contain recursive structures' if entered_object_ids.key?(object_id)
        ...
        attr_reader :entered_object_ids, :error_class
      end
      ...
      class NormalizedGraph
        def initialize(value, error_class:, immutable_scalar_checker:, duplicable_scalar_checker:)
          @value = value
          @error_class = error_class
        ...
        def normalized?
          normalized_value?(value, tracker: TraversalTracker.new(error_class:))
        end

        private

        attr_reader :duplicable_scalar_checker, :error_class, :immutable_scalar_checker, :value
