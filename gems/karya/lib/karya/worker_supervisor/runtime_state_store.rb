  ...
  class WorkerSupervisor
    ...
    class RuntimeStateStore
      ...
      def update_payload(initial_payload: nil)
        ...
        File.open(lock_path, File::RDWR | File::CREAT, FILE_MODE) do |lock_file|
      ...
      def ensure_parent_directory
        FileUtils.mkdir_p(File.dirname(path), mode: DIRECTORY_MODE)
      end
      ...
      class AtomicPayloadWriter
        ...
        def write
          Tempfile.create(['karya-runtime-state', '.json'], File.dirname(@path)) do |tempfile|
            @tempfile = tempfile
            File.chmod(FILE_MODE, @tempfile.path)
            write_payload
            flush_payload
            publish_payload
          end
