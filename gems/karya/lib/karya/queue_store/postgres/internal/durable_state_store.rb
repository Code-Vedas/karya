...
module Karya
  module QueueStore
    class Postgres
      module Internal
        # Postgres-backed row reader/writer for the durable queue-store schema.
        class DurableStateStore < Karya::Internal::DurableQueueStore::SqlStateStore
          ...
          def select_all(sql, binds)
            connection.exec_params(sql, binds).to_a.map do |row|
          ...
          def normalize_value(column_name, value)
            ...
            return value.utc if TIME_COLUMNS.include?(column_name) && value.is_a?(Time)
