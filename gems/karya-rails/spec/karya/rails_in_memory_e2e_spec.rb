...
RSpec.describe Karya::Rails, :dashboard_manifest_fixture, :integration do
  ...
  it 'uses the configured InMemory backend through the mounted engine' do
  ...
  it 'enqueues ActiveJob payloads through the Rails compatibility path' do
    ...
    reservation = queue_store.reserve(queue: 'dashboard', worker_id: 'rails-active-job-worker', lease_duration: 60, now: Time.now.utc + 1)
    running_job = queue_store.start_execution(reservation_token: reservation.token, now: Time.now.utc + 2)
