...
module Karya
  module QueueStore
    class Postgres
      module Internal
        # Encodes the durable queue-store state payload stored in Postgres.
        module StateCodec
          module_function
          ...
          def load(payload)
            ...
            raise InvalidQueueStoreOperationError, "invalid Postgres state snapshot: #{e.class}: #{e.message}", cause: e
          end
          ...
          def validate_snapshot(snapshot)
            unless snapshot.is_a?(Hash) &&
              ...
              raise InvalidQueueStoreOperationError, 'invalid Postgres state snapshot payload'
            end

            snapshot
          rescue KeyError => e
            raise InvalidQueueStoreOperationError, "invalid Postgres state snapshot: #{e.message}", cause: e
          end
        end
      end
    end
