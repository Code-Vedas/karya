    ...
    class InMemory
      module Internal
        ...
        module ExpirationSupport
          # Encapsulates expiry checks and failure transition for one stored job.
          class ExpiringJob
            def initialize(job)
              @job = job
            end
            ...
            def to_failed_job(now)
              @job.expire(updated_at: now)
            end
          end

          private_constant :ExpiringJob
          ...
          def job_expired?(job, now)
            ExpiringJob.new(job).expired?(now)
          end

          def build_expired_job(job, now)
            ExpiringJob.new(job).to_failed_job(now)
          end
        end
      end
    end
