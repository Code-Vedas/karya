...
RSpec.describe 'Karya::Reservation::IdentifierNormalizer' do
  let(:identifier_normalizer_class) { Karya::Reservation.const_get(:IdentifierNormalizer, false) }

  describe '#normalize' do
    it 'converts to string and strips whitespace' do
      normalizer = identifier_normalizer_class.new(:token, '  tok123  ')
      expect(normalizer.normalize).to eq('tok123')
    end

    it 'converts symbol to string' do
      normalizer = identifier_normalizer_class.new(:job_id, :job456)
      expect(normalizer.normalize).to eq('job456')
    end

    it 'converts integer to string' do
      normalizer = identifier_normalizer_class.new(:worker_id, 789)
    ...
    it 'raises InvalidReservationAttributeError for blank value' do
      expect do
        identifier_normalizer_class.new(:token, '   ').normalize
    ...
    it 'raises InvalidReservationAttributeError for empty string' do
      expect do
        identifier_normalizer_class.new(:job_id, '').normalize
    ...
    it 'handles strings that become blank after stripping' do
      expect do
        identifier_normalizer_class.new(:queue, "\n\t  \r\n").normalize
