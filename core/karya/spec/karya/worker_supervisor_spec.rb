...
RSpec.describe Karya::WorkerSupervisor do
  ...
  describe '#run' do
    ...
    it 'does not write runtime phases from signal handlers before the run is claimed' do
      runtime_state_store = instance_double(
        described_class.const_get(:RuntimeStateStore, false),
        mark_supervisor_phase: nil
      )
      allow(described_class.const_get(:WakeupSignal, false)).to receive(:interrupt)
      supervisor_with_runtime_state = described_class.new(
        queue_store: queue_store,
        worker_id: 'worker-supervisor',
        queues: ['billing'],
      ...
      expect do
        supervisor_with_runtime_state.send(
          :handle_shutdown_signal,
          Karya::Internal::RuntimeSupport::ShutdownState.new
        )
    ...
    it 'does not abort signal handling when runtime phase persistence fails' do
      runtime_state_store = instance_double(
        described_class.const_get(:RuntimeStateStore, false),
        mark_supervisor_phase: nil
      )
      allow(described_class.const_get(:WakeupSignal, false)).to receive(:interrupt)
      allow(runtime_state_store).to receive(:mark_supervisor_phase)
        .and_raise(described_class.const_get(:InvalidRuntimeStateFileError, false), 'boom')
      supervisor_with_runtime_state = described_class.new(
        queue_store: queue_store,
        worker_id: 'worker-supervisor',
        queues: ['billing'],
      ...
      expect(runtime_state_store).to have_received(:mark_supervisor_phase).with('draining')
    end
